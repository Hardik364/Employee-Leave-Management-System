/** Employee dashboard: summary stats + recent activity. */
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Your leave overview</p>
        </div>
        <Link to="/apply" className="btn-primary">Apply for Leave</Link>
      </div>

      <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
        <StatCard label="Total" value={t.total} accent="brand" />
            ...
            {stats.recent.map((l) => (
              <li key={l.id} className="flex items-center justify-between py-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link to={`/leaves/${l.id}`} className="text-sm text-brand-600 hover:underline">
                    View
                  </Link>
                </div>
              </li>
